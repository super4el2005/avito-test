import { Accordion, ActionIcon, Badge, Button, Card, Checkbox, Container, Group, Select, SimpleGrid, Text, Image, Stack, Switch, TextInput, Title, Pagination, Loader, Skeleton, Alert, ScrollArea } from '@mantine/core';
import { useQuery } from '@tanstack/react-query';
import { apiAds } from '~/api';
import { MdGridView, MdFormatListBulleted, MdSearch, MdOutlineClear, MdInfo } from "react-icons/md";
...
export default function () {
    ...
    const getAdsQuery = useQuery({
        ...
        queryFn: ({ signal }) => apiAds.get<ResponseAds>('/items', {
            params: {
        ...
        <Stack>
            ...
            <Group>
                <TextInput
                    ...
                    rightSection={getAdsQuery.isLoading ? <Loader size={20} /> : form.values.q ? <ActionIcon onClick={() => form.setFieldValue('q', '')} variant="light" color="gray" aria-label="Settings">
                        <MdOutlineClear size={20} />
                    </ActionIcon> : <MdSearch size={20} />}
                    key={form.key('q')}
                    {...form.getInputProps('q')}
                />
                <ActionIcon.Group >
                    <ActionIcon radius="md" onClick={() => setViewMode('grid')} variant={viewMode === "grid" ? "outline" : "default"} size="lg" aria-label="Gallery">
                        <MdGridView size={20} />
                    </ActionIcon>

                    <ActionIcon radius="md" onClick={() => setViewMode('list')} variant={viewMode === "list" ? "outline" : "default"} size="lg" aria-label="Settings">
            ...
            <Group align="flex-start">
                <Stack w={200}>
                    ...
                    <Accordion defaultValue="category">
                        <Accordion.Item value={"category"}>
                            ...
                            <Accordion.Panel>
                                ...
                                >
                                    <Stack mt="xs">
                                        {CATEGORIES_FORM.map(category => <Checkbox key={category.value} value={category.value} label={category.label} />)}
                                    </Stack>
                                </Checkbox.Group>
                            </Accordion.Panel>
                        </Accordion.Item>
                    ...
                    />
                    <Button disabled={!isFullyForm} onClick={() => form.setValues({
                        q: "",
                        categories: [],
                        needsRevision: false,
                ...
                <Stack flex={1}>
                    {!getAdsQuery.data?.data.items.length && !getAdsQuery.isLoading && <Alert icon={<MdInfo />} title="Ничего не найдено">
                    </Alert>}
                    {getAdsQuery.isError && <Alert color='red' icon={<MdInfo />} title="Ошибка">
                    </Alert>}
                    {viewMode === 'grid' && <SimpleGrid cols={5} h={650}>
                        {!isDataLoading && !!getAdsQuery.data?.data.items.length && getAdsQuery.data.data.items.map((ad) =>
                            <Card
                                key={ad.id}
                                h={300}
                                shadow="sm"
                        ...
                        {isDataLoading && [...new Array(LIMIT_ADS)].map((_, id) =>
                    ...
                    {viewMode === 'list' && <ScrollArea h={650}>
                        <Stack>
                            {!isDataLoading && !!getAdsQuery.data?.data.items.length && getAdsQuery.data.data.items.map((ad) =>
                                <Card
                                    key={ad.id}
