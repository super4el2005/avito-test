...
- `domain` — бизнес-логика и доменные сущности конкретного продукта; может использовать `shared`.
- `routes` — слой orchestration: собирает страницу из `domain` + `shared`, связывает данные, UI и навигацию.
